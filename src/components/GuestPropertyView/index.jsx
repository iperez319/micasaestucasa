...
class GuestPropertyView extends Component{
    ...
    render(){
        ...
        const pendingProposalsdataSource = property.attrs.pendingProposals.map((value, key) => {
            return {
                ...
                endDate: value.endDate,
                pricePerNight: "$" + value.price,
                total: "$" + (((new Date(value.endDate) - new Date(value.startDate))/(1000*60*60*24)) - 1) * value.price,
                user: value.user,
        ...
        return (
            <div>
                ...
                <Divider/>
                <Title level={3} style={{marginBottom: '25px'}}>Approximate Location</Title>
                <MapContainer guest googleMapURL="https://maps.googleapis.com/maps/api/js?v=3.exp&libraries=geometry,drawing,places" loadingElement={<div style={{ height: `100%` }} />} containerElement={<div style={{ height: `400px` }} />} mapElement={<div style={{ height: `100%` }} />} lat={property.attrs.lat} lng={property.attrs.lng}/>
                <Divider/>
...
export default GuestPropertyView